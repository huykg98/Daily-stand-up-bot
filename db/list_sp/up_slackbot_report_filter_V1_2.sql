...
DELIMITER //
DROP PROCEDURE IF EXISTS up_slackbot_report_filter_V1_2//
CREATE PROCEDURE `up_slackbot_report_filter_V1_2`(
							pFromDate BIGINT(8), pToDate BIGINT(8), 
							pUsername VARCHAR(45), pChannel VARCHAR(45))
BEGIN
	SET @queryStr = CONCAT('
	select sdd.label, scd.value, temp.user_name, temp.timestamp, temp.avatar
